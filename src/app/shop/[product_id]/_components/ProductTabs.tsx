...
export default function ProductTabs({ product }: ProductTabsProps) {
  ...
  const video = useMemo(() => {
    return product.media?.find((item) => item.media_type === "YOUTUBE_LINK") as
      | ProductYouTubeMedia
      | undefined;
  }, [product]);
        ...
        <div className="grid grid-cols-2 gap-3 lg:hidden">
          ...
          <div className="col-span-2 flex justify-center">
            <button
              ...
              className={`w-1/2 px-4 py-3 text-sm font-medium transition-all rounded-xl ${
                activeTab === tabs[2].id
                  ? "bg-[#003C22] text-white"
                  : "bg-[#E8F7F0] text-[#003C22]"
              }`}
        ...
        {activeTab === "details" && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
            ...
            <div className="space-y-4 lg:space-y-6">
              ...
              {product.properties?.features_and_benefits &&
                Array.isArray(product.properties.features_and_benefits) && (
                  <div>
                    <h4 className="text-base lg:text-lg font-semibold text-gray-900 mb-2 lg:mb-3">
                      Key Features & Benefits
                    </h4>
                    <ul className="list-disc list-inside space-y-2 text-gray-600 text-sm">
                      {product.properties.features_and_benefits.map(
            ...
            <div className="overflow-x-auto rounded-lg border border-gray-200">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-200">
                  ...
                  <tr className="bg-[#E0E2E7] hover:bg-gray-50">
                    <td className="px-3 lg:px-6 py-3 lg:py-4 text-xs lg:text-sm text-gray-600">
                      Brand
                    </td>
                    <td className="px-3 lg:px-6 py-3 lg:py-4 text-xs lg:text-sm text-gray-900">
                  ...
                  {Object.entries(groupedTags).map(([tagName, values]) => (
                    <tr
                      key={tagName}
                      className="odd:bg-white even:bg-[#E0E2E7] hover:bg-gray-50"
                    >
                      <td className="px-3 lg:px-6 py-3 lg:py-4 text-xs lg:text-sm text-gray-600">
                        {tagName}
                      </td>
