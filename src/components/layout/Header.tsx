...
import { useAuth } from '@/context/AuthContext'; 
import LoginPopup from '@/components/auth/LoginPopup';
import { useFilterData } from '@/hooks/useFilterData'; // [!code ++] Import Hook
...
const Header = () => {
  ...
  // [!code ++] USE THE HOOK (Replaces manual fetch)
  const { data: navData } = useFilterData();
  ...
  // [!code --] REMOVED: The manual useEffect fetching logic
  /*
  useEffect(() => {
    const loadData = async () => { ... }
    loadData();
  ...
  */
  ...
  // --- Scroll Lock ---
  useEffect(() => {
    if (isMobileMenuOpen || isLoginOpen || showLogoutConfirm) {
      document.body.style.overflow = 'hidden';
    } else {
      document.body.style.overflow = 'unset';
    }
  }, [isMobileMenuOpen, isLoginOpen, showLogoutConfirm]);
  ...
  return (
    ...
    {showLogoutConfirm && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden font-jakarta transform scale-100 transition-all">
            <div className="p-6 text-center">
              ...
              <div className="flex gap-3">
                ...
                <button 
                  ...
                  className="flex-1 py-2.5 rounded-xl bg-red-600 text-white font-semibold hover:bg-red-700 transition-colors text-sm shadow-sm shadow-red-200"
                >
                  Yes, Logout
                </button>
              </div>
