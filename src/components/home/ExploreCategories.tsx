...
// [!code highlight] 1. Import your reusable component
import AnimatedPress from '@/components/shared/AnimatedPress'; 
...
const ExploreCategories = () => {
  ...
  const handleNavigate = (categoryName: string) => {
    // Optional: Add a tiny delay here if AnimatedPress doesn't handle it internally
    // so the user sees the shrink animation before the page changes.
    setTimeout(() => {
  ...
  return (
    <section className="w-full bg-white font-jakarta flex items-center justify-center py-12 xl:py-0 xl:h-[428px]">
      <div className="w-full max-w-[1600px] px-4 xl:px-0 flex flex-col items-center">
        ...
        >
          <span className="text-[20px] font-bold md:text-[28px] xl:text-[28px]">Explore Categories</span>
        </h2>
        ...
        ">
          {CATEGORIES.map((cat) => (
            // [!code highlight] 2. Reuse AnimatedPress here
            <AnimatedPress
              key={cat.id}
              ...
              "
            >
              {/* --- CARD CONTENT (Kept exactly same) --- */}
              <div 
                className="
                  flex flex-col items-center justify-center
                  w-[86px] h-[102px] gap-[8px]
                  md:w-[130px] md:h-[160px] md:gap-[10px]
                  xl:w-[162px] xl:h-[192px] xl:gap-[12px]
                "
              >
