        ...
        <div className="relative w-full h-auto lg:h-[817px] bg-[#FD820B]/10 overflow-hidden">
          ...
          <div className="relative z-10 w-full h-full px-4 xl:px-0 py-12 max-w-[1296px] mx-auto flex flex-col items-center lg:items-stretch">
            ...
            <div className="hidden lg:flex gap-6 h-full w-full">
               {bundles[0] && (
                  // Using % width for fluidity on smaller laptops, max-width to respect original design
                  <div className="flex-[0_0_40%] xl:flex-[0_0_504px] h-[517px] shrink-0">
                      <LargeCard item={bundles[0]} onBuy={() => handleBuyClick(bundles[0].id)} />
                  </div>
               )}
               <div className="flex flex-col gap-6 flex-1 min-w-0">
                  <div className="flex gap-6 h-[317px] w-full">
                     {bundles.slice(1,4).map(b => (
                         // Flex-1 allows cards to fill available space evenly
            ...
            <div className="flex lg:hidden flex-col items-center gap-4 w-full">
                {bundles[0] && (
                    // Responsive width: max-w-full for phones, max-w-[768px] for tablets
                    <div className="w-full max-w-[369px] md:max-w-[768px] h-auto shrink-0">
                        <LargeCard item={bundles[0]} onBuy={() => handleBuyClick(bundles[0].id)} />
                    </div>
                )}
                
                {/* Tablet Optimization: Switches to 4 columns on medium screens */}
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 w-full max-w-[369px] md:max-w-[768px]">
                    {bundles.slice(1, 5).map(b => (
