...
//           {/* [!code modified] Always render this block. Use invisible text if no discount to reserve height. */}
//           {item.discount > 0 ? (
...
// interface CartItemProps {
//   item: CartItemType;
//   onUpdate?: () => void; // [!code ++] New optional callback
// }
...
//              await addToCart(item.product_variant_id, delta);
//              lastSyncedQty.current = newQty;
//              if (onUpdate) onUpdate(); // [!code ++] Trigger refresh
//            } catch (err) {
//              console.error("Cart Item update failed", err);
//              setQuantity(lastSyncedQty.current);
//            } finally {
...
//   item: CartItemType;
//   onUpdate?: () => void;
//   readonly?: boolean; // [!code ++] New prop to disable editing
// }

// const CartItem = ({ item, onUpdate, readonly = false }: CartItemProps) => {
//   const router = useRouter();
//   const { addToCart, removeFromCart } = useCart();
...
//   const handleRemove = async () => {
//     if (readonly || isRemoving) return; // Prevent action if readonly
//     setIsRemoving(true);
//     try {
//       await removeFromCart(item.product_variant_id);
//       if (onUpdate) onUpdate();
