...
const FilterBar = ({ filters, onFilterChange }: FilterBarProps) => {
  ...
  const handleReset = () => {
    onFilterChange({ 
      ...
      size: undefined,
      uom: undefined,
      sortBy: undefined // [!code ++] Reset Sort too
    });
    setOpenDropdown(null);
  };

  // [!code ++] Helper to get current sort label
  const getSortLabel = () => {
    const active = SORT_OPTIONS.find(opt => opt.value === filters.sortBy);
  ...
  // [!code ++] Reusable Sort Dropdown Content
  const SortDropdownContent = () => (
    <>
      <MobileBackdrop />
      <div className={dropdownContainerClass}>
        <div className="w-full flex justify-center py-3 lg:hidden bg-gray-50 border-b border-gray-100">
          <div className="w-12 h-1.5 bg-gray-300 rounded-full"></div>
        </div>
        <div className="p-2">
           <div className="text-[10px] font-bold text-gray-400 mb-2 px-2 pt-2">SORT ORDER</div>
           {SORT_OPTIONS.map((opt) => (
             <div 
               ...
               className={`flex items-center justify-between px-3 py-3 lg:py-2 cursor-pointer rounded-md text-sm mb-1 ${filters.sortBy === opt.value ? 'bg-[#013220]/10 text-[#013220] font-medium' : 'hover:bg-gray-50 text-gray-700'}`}
             >
               {opt.label} 
               {filters.sortBy === opt.value && <Check size={14} />}
             </div>
           ))}
        </div>
      </div>
        ...
        <div className="w-full"> 
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 w-full">
            ...
            >
              ...
              {/* Pack Size (Replaced 'Application') */}
              <div className="relative">
                <button onClick={() => toggleDropdown('PackSize')} className={`bg-white border ${(filters.size !== undefined || filters.uom !== undefined) ? 'border-[#013220] text-[#013220]' : 'border-gray-200 text-[#000000]'} rounded-lg flex items-center justify-center transition-colors ${buttonTextClasses}`} style={buttonStyle}>
                  {getSizeLabel()} 
                  <img src="/icons/arrow-down.svg" alt="down" className={`transition-transform duration-200 ${openDropdown === 'PackSize' ? 'rotate-180' : ''}`} style={{ width: '20px', height: '20px' }} />
              ...
              {/* [!code ++] Mobile Only: Sort */}
              <div className="flex items-center gap-3 lg:hidden">
                <div className="relative">
                    <button 
                      onClick={() => toggleDropdown('SortMobile')}
                      className={`bg-white border ${filters.sortBy ? 'border-[#013220] text-[#013220]' : 'border-gray-200'} rounded-lg flex items-center justify-center hover:border-[#013220] ${buttonTextClasses}`} 
                      style={buttonStyle}
                    >
                      {getSortLabel()}
                      <img src="/icons/arrow-down.svg" alt="down" className={`transition-transform duration-200 ${openDropdown === 'SortMobile' ? 'rotate-180' : ''}`} style={{ width: '20px', height: '20px' }} />
                    </button>
                    {openDropdown === 'SortMobile' && <SortDropdownContent />}
                </div>
            ...
            {/* [!code ++] Desktop Only: Sort & Reset */}
            <div className="hidden lg:flex items-center gap-3">
              
              {/* Sort Button */}
              <div className="relative">
                <button 
                  onClick={() => toggleDropdown('SortDesktop')}
                  className={`bg-white border ${filters.sortBy ? 'border-[#013220] text-[#013220]' : 'border-gray-200'} rounded-lg flex items-center justify-center gap-2 hover:border-[#013220] transition-colors ${buttonTextClasses}`} 
                  style={buttonStyle}
                >
                  {getSortLabel()}
                  <img src="/icons/arrow-down.svg" alt="down" className={`transition-transform duration-200 ${openDropdown === 'SortDesktop' ? 'rotate-180' : ''}`} style={{ width: '20px', height: '20px' }} />
                </button>
                {openDropdown === 'SortDesktop' && <SortDropdownContent />}
              </div>
