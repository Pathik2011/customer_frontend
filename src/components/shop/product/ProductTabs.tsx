...
const ProductTabs = ({ product }: ProductTabsProps) => {
  ...
  // [!code ++] Helper to safely render property values (Handles String, Array, and Object)
  const renderValue = (value: any) => {
    ...
    if (typeof value === 'object' && value !== null) {
      return (
        <div className="flex flex-col gap-1 text-[#4D4D4D]">
           {Object.entries(value).map(([subKey, subVal]) => (
              <div key={subKey} className="flex gap-2">
                 <span className="font-semibold capitalize text-black/80">{subKey.replace(/_/g, ' ')}:</span>
    ...
    return <p className="text-[#4D4D4D]">{value}</p>;
  };
